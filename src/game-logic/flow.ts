...
import { structuredCloneGame, sortTiles, withoutTiles, tableNarration, appendAction } from "./helpers";
...
export function advanceAfterDiscard(
  ...
  if (hu !== undefined) {
    if (isHumanSeat(hu)) {
      next.pendingClaim = {
        ...
        canChi: (discardedBy + 1) % 4 === hu && possibleChiOptions(next.players[hu].hand, discard.tile).length > 0,
      };
      next.phase = "claim";
      next.message = `${next.players[discardedBy].name} discarded ${discard.tile.label}. ${next.players[hu].name} can win on this discard.`;
      next.activity = { player: discardedBy, text: `${next.players[hu].name} can win on this discard.`, tile: discard.tile };
      return next;
    }
  ...
  for (const playerIndex of order) {
    ...
    if (isHumanSeat(playerIndex)) {
      next.pendingClaim = {
        ...
        canChi: (discardedBy + 1) % 4 === playerIndex && possibleChiOptions(next.players[playerIndex].hand, discard.tile).length > 0,
      };
      next.phase = "claim";
      ...
      next.activity = { player: discardedBy, text: `${next.players[playerIndex].name} can claim or pass.`, tile: discard.tile };
      return next;
    }
  ...
  if (isHumanSeat(chiSeat) && possibleChiOptions(next.players[chiSeat].hand, discard.tile).length > 0) {
    next.pendingClaim = {
      tile: discard.tile,
      by: discardedBy,
      claimer: chiSeat,
    ...
    next.activity = { player: discardedBy, text: `${next.players[chiSeat].name} can Chi or pass.`, tile: discard.tile };
    return next;
  }

  const chiPlayer = (discardedBy + 1) % 4;
  if (!isHumanSeat(chiPlayer) && possibleChiOptions(next.players[chiPlayer].hand, discard.tile).length > 0 && shouldCall(next.players[chiPlayer], "chi")) {
...
export function applyClaim(
  ...
  if (type === "pong") {
    ...
    const chiTiles = chosenTiles ?? possibleChiOptions(player.hand, discard.tile)[0];
    if (!chiTiles) return next;
    player.hand = sortTiles(withoutTiles(player.hand, chiTiles.filter((tile: Tile) => tile.id !== discard.tile.id)));
  ...
  next.message = tableNarration("claim", player.name, type === "kong" ? "Kong" : type.charAt(0).toUpperCase() + type.slice(1));
  next.activity = { player: playerIndex, text: next.message };
  appendAction(next, "claim", playerIndex, next.message);

  if (type === "kong") {
    if (next.wall.length === 0) {
      return { ...next, phase: "round-over" as any, message: tableNarration("draw", "") };
    }
...
export function startTurn(
  ...
  if (game.wall.length === 0) {
    return { ...game, phase: "round-over" as any, message: tableNarration("draw", "") };
  }
  ...
  if (kongCode && !isHumanSeat(playerIndex) && Math.random() < (player.difficulty === "sharp" ? 0.75 : 0.35)) {
...
export function applyKong(game: Game, playerIndex: number, code: string, concealed: boolean, rules: Rules, houseRules: HouseRule[]) {
  let next = structuredCloneGame(game);
  const player = next.players[playerIndex];
...
export function applyConcealedKong(game: Game, playerIndex: number, code: string, rules: Rules, houseRules: HouseRule[]) {
